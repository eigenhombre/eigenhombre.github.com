...
<code>$ conttest nosetests  # Runs nosetests whenever the files on disk change</code>

<code>$ conttest nosetests path.to.test:harness  # Runs only tests in 'harness' object in path/to/test </code>

<code>$ conttest 'pep8 -r . ; nosetests'  # Check both PEP-8 style and unit tests</code>

It would work equally well with a different language ('"blub":http://www.paulgraham.com/avg.html') with a separate compilation step:

<code>$ conttest 'make && ./run-tests'</code>
