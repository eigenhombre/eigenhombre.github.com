...
Love 'em or hate 'em, Lisp code has lots of parentheses (though Clojure has less than some).  I come to Clojure with a long background in Python, where whitespace plays an important role in enhancing readability (no curly braces or semicolons to establish block level and to separate statements).  In graphic design, whitespace is a key aspect of composition; its use in code is a __graphical trick which enhances readability__.
