...
Now edit project.clj, including a dependency on "Incanter":http://incanter.org.  It is apparently common to download stand-alone installations of Incanter, but we would like to have the option of using other Clojure libraries such as "seesaw":https://github.com/daveray/seesaw, so we'll download the dependency directly:
...
Now we can actually start to do something.  Download the CVS file titled @train@ from "the Data Files page":http://www.kaggle.com/c/digit-recognizer/data, saving it in your home directory as @train.csv@.
...
(->>
  *trainfile*
  ...
  (drop 1)               ;; Drop first line, which is the text header
...
Now that we know how to do basic passes through the file, let's start to actually train our classifier.  We are going to minimize our cost function using batch gradient descent, which involves calculating the derivatives
